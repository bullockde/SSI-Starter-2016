	...
	<form method="post" action="">		
		<div class='clear'></div>
				...
				<div class='col-sm-2' >
					...
					<input type="text" name="lead_phone" placeholder="Phone"  value="<?php echo get_field( 'lead_phone', $lead->ID ); ?>">
				</div>
				<div class='col-sm-3' >
					<b>Email: </b> 
					
					
					<input type="text" name="lead_email" placeholder="Email" value="<?php echo get_field( 'lead_email', $lead->ID ); ?>">
				...
				<div class='col-sm-12'> 
					...
					<div class='col-sm-2'>
						...
						<input type="text" name="lead_city" placeholder="City" Value="<?php echo get_field( 'lead_city', $lead->ID ); ?>">

					</div>
					<div class='col-sm-1'>

						<input type="text" name="lead_state" placeholder="State" value="<?php echo get_field( 'lead_state', $lead->ID ); ?>">
...
<div class='clear'></div>
	...
	<div id='details' class='details' style='display: block;'>	
				...
				<b>D.O.B: </b> <input width="100" type="text" name="lead_dob" placeholder="D.O.B" placeholder="<?php echo get_field( 'lead_dob', $lead->ID ); ?>">
...
<div class='clear'></div>
	...
	// The Query
				...
				$args = array( 'post_type' => 'ssi_clients'  , 'posts_per_page' => $num_posts, 'orderby'=> 'modified', 'order' => $order  );
				
				//$args = array( 'post_type' => 'ssi_clients'  , 'posts_per_page' => $num_posts, 'orderby'=> 'meta_value_num', 'meta_key' => 'client_profit' ,'order' => $order  );
		...
		/*	foreach( $leads as $lead ){
			...
			*/
			...
			foreach( $leads as $lead ){
				...
				$searchword = get_field( 'lead_phone', $lead->ID );
				...
				}else if( strpos( get_field( 'lead_phone', $lead->ID ) , strtolower($_GET['searchterm']) ) ){ 
					array_push( $filtered, $lead );
					//echo "Not FOUND -->" . $_GET['searchterm'];
				}else{
...
<!--  #####################   END Filter  ##############-->
	...
	<div class='col-sm-12 hidden-xs text-center'>
		...
		<div class='col-xs-2 '> Social </div><br>
		<div class='clear'></div><HR>
	</div>
...
<div class='col-xs-4 col-sm-2'> 
					...
					<div class='col-xs-4 col-sm-2'>
						...
						<a href="tel:<?php echo get_field( 'lead_phone', $lead->ID ); ?>"><?php echo get_field( 'lead_phone', $lead->ID ); ?></a>
...
if ( ! empty( $categories ) ) {
	...
	if( $categories[0]->name == 'Friend' ){
		...
		$cnt_family++;
		set_post_type( $lead->ID, 'ssi_family' );
...
<div id='details<?php echo $lead->ID; ?>' class='details' style='display: none;'>
	...
	<div class=' col-xs-12'>			
				...
				<div class='col-sm-12' >
					...
					<input width="100" type="text" name="lead_dob" placeholder="D.O.B" value="<?php echo get_field( 'lead_dob', $lead->ID ); ?>">
				</div>
	...
	<div id='add_payment' class='' style='display: none;'>
		...
		<div class='col-xs-6 col-sm-1'>
			...
			<?php 
				...
				<div class='col-sm-3'><div class='pull-right'><?php echo "Total: $" . $client_profit; 
				
				//if( $client_profit > 0 ){ set_post_type( $lead->ID, 'ssi_clients' ); }
...
<?php 
		...
		foreach( $social as $lead ){
			...
			?>		
				...
				<div class="hidden-xs col-sm-1 pad0"><a target='_blank' href='<?php echo get_field( 'website_link' , $lead->ID ); ?><?php echo get_field( $lead->post_name , $lead->ID ); ?>'><img src='
